...
if(@ARGV != 6)
{
        print "Example: perl Split_SingleTube_reads.2.pl barcode.list barcode_RC.list read_1.fq.gz read_2.fq.gz 100 split_read \n";
        exit(0);
}
...
while(<IN2>){
  ...
  if($n % 4 == 2){
    ...
    if((exists $barcode_hash{$b1}) && (exists $barcode_hash{$b2}) && (exists $barcode_hash{$b3})){
      ...
      $Read1[$index_hash{$hash}][$Read_num[$index_hash{$hash}]] = $id."\#$hash\/2\t$index_hash{$hash}\t1";
