...
if (@ARGV != 6) {
  print
    "Example: perl Split_SingleTube_reads.2.pl barcode.list barcode_RC.list read_1.fq.gz read_2.fq.gz 100 split_read \n";
  exit(0);
}
...
while (<IN2>) {
  ...
  if ($n % 4 == 1) {
    ...
    if ($reads_num % 1000000 == 1) {
      print "reads_2 processed $progress (M) reads ...\n";
      $progress++;
    }
  ...
  if ($n % 4 == 2) {
    ...
    if (   (exists $barcode_hash{$b1})
        && (exists $barcode_hash{$b2})
        && (exists $barcode_hash{$b3}))
    {
      my $hash =
          $barcode_hash{$b1} . "_"
        . $barcode_hash{$b2} . "_"
      ...
      $Read1[$index_hash{$hash}][$Read_num[$index_hash{$hash}]] =
        $id . "\#$hash\/2\t$index_hash{$hash}\t1";
      $Read2[$index_hash{$hash}][$Read_num[$index_hash{$hash}]] = $read;
      $T = <IN2>;
      $n++;
      chomp($T);
      ...
      $Read4[$index_hash{$hash}][$Read_num[$index_hash{$hash}]] = $qual;
    } else {
      $Read_num[0]++;
      $Read1[0][$Read_num[0]] = $id . "\#0_0_0\/2\t0\t1";
      $Read2[0][$Read_num[0]] = $read;
      $T                      = <IN2>;
      $n++;
      chomp($T);
      ...
      chomp($T);
      my $qual = substr($T, 0, $read_len);
      $Read4[0][$Read_num[0]] = $qual;
    }
  }
}
close IN2;

open OUT2, ">$ARGV[5]\_split_stat_read2.log" or die "Can't write file";
open OUT,  "| gzip > $ARGV[5].2.fq.gz"       or die "Can't write file";
print OUT2
  "Barcode_types = $barcode_each * $barcode_each * $barcode_each = $barcode_types\n";
my $r;
